...
```bash
podman build --build-arg OTEL_VERSION=$OTEL_VERSION -t otelcollector-syslog:$OTEL_VERSION .
```

Push image to the rigistry with podman:
### Push image
```bash
podman push otelcollector-syslog:$OTEL_VERSION <registry>/<namespace>/otelcollector-syslog:$OTEL_VERSION
```
